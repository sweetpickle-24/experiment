...
# The scored suite. Defaults are the configuration reported above.
...
**The defaults are the pipeline reported above**, so a bare run reproduces these
results. Expect a few hours on CPU; the learning benchmark and its learning-rate
sweep dominate.

To reproduce the superseded 2/5 baseline instead, set all three back:
...
Every result file records which pipeline actually ran, so a file cannot misreport
itself, and `scripts/run_repaired_suite.py` refuses to combine files that disagree
on configuration.
